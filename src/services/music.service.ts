		...
		this.player
			.on(
				...
				async (queue: Queue, playlist: Playlist<unknown>) => {
					this.logger.info(
						`Added playlist! Playlist Name: ${playlist.name} | Queue length: ${queue.songs.length}`
					);
				}
			)
			.on("addSong", async (queue: Queue, song: Song<unknown>) => {
				this.logger.info(
					`Added song! Song name: ${song.name} | Queue length: ${queue.songs.length}`
				);
			})
			.on("deleteQueue", async (queue: Queue) => {
				this.logger.info(
					`Deleted queue! Voice Channel: ${queue.voiceChannel?.name} | Guild: ${queue.textChannel?.guild.name}`
				);
			})
			.on("disconnect", async (queue: Queue) => {
				this.logger.info(
					`Disconnected! Voice Channel: ${queue.voiceChannel?.name} | Guild: ${queue.textChannel?.guild.name}`
				);
			})
			.on("empty", async (queue: Queue) => {
				this.logger.info(
					`Channel Empty! Voice Channel: ${queue.voiceChannel?.name} | Guild: ${queue.textChannel?.guild.name}`
				);
			})
			.on(
				"error",
			...
			.on("finish", async (queue: Queue) => {
				this.logger.info(
					`Queue finished! Voice Channel: ${queue.voiceChannel?.name} | Guild: ${queue.textChannel?.guild.name} `
				);
			})
			.on("finishSong", async (queue: Queue, song: Song<unknown>) => {
				this.logger.info(
					`Song finished! Song: ${song.name} Voice Channel: ${queue.voiceChannel?.name} | Guild: ${queue.textChannel?.guild.name} `
				);
			})
			.on("initQueue", async (queue: Queue) => {
				this.logger.info(
					`Queue initialized! Voice Channel: ${queue.voiceChannel?.name} | Guild: ${queue.textChannel?.guild.name} `
				);
			})
			.on("noRelated", async (queue: Queue) => {
				this.logger.info(
					`No related? Voice Channel: ${queue.voiceChannel?.name} | Guild: ${queue.textChannel?.guild.name} `
				);
			})
			.on("playSong", async (queue: Queue, song: Song) => {
				this.logger.info(
					`Playing song! Name: ${song.name} | Queue length: ${queue.songs.length}`
				);
			})
			.on(
				"searchCancel",
